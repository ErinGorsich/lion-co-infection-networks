...
make_matrix= function(data){
    tmat<-matrix(NA, nrow=length(data[,1]), ncol=21,  # makes lions columns 
        dimnames=list(data$LION, colnames(colnames(plotdata[-1])))) # all inf -FIV
...
make_all_bipartite_matrix= function(data){
    tmat<-matrix(0, ncol=length(c(as.character(data$LION), colnames(plotdata))), 
        nrow=length(c(as.character(data$LION), colnames(plotdata))), 
            dimnames=list(
...
make_bipartite_matrix= function(data){
    tmat<-matrix(0, 
        ncol = length(c(as.character(data$LION), colnames(plotdata)) )-1, 
        nrow = length(c(as.character(data$LION), colnames(plotdata)))-1 , 
        dimnames = list(
